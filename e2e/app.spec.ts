...
test.describe('Sub Tracker App', () => {
  test('should load the homepage and display correct content', async ({
    page,
  }) => {
    await page.goto('/')
    ...
    await expect(
      page.getByRole('heading', { name: 'Sub Tracker' })
    ).toBeVisible()

    // Check for the subtitle
    await expect(
      page.getByText('Track your subscriptions by connecting to YNAB')
    ).toBeVisible()
